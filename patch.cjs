...
code = code.replace(
  /groupItems\.map\(\(item\) => \(\s*<div key=\{item\.id\} className="bg\[#0B1221\] border border-slate-800 rounded-xl p-3 flex justify-between items-center group\/item hover:border-slate-700 transition-colors">/g,
  `groupItems.map((item) => (
...
code = code.replace(
  /<div className="mt-4 pt-4 border-t border-slate-800">\s*<button/g,
  `{(() => {
                  ...
                  return (
                    <div className="mt-4 pt-4 border-t border-slate-800 flex gap-2">
                      ...
                      >
                        <Send className="w-3.5 h-3.5" /> Enviar Tudo
                      </button>
                      <button
                        onClick={() => {
