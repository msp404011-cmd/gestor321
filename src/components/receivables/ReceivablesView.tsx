...
const EditReceivableModal: React.FC<EditReceivableModalProps> = ({
  isOpen,
  receivable,
  onClose,
  onSuccess,
  ...
  const handleSave = (e: React.FormEvent) => {
    e.preventDefault();
    const list = StorageService.getReceivables();
    const updated = list.map((r) => (r.id === receivable.id ? {
      ...r,
      customerName,
      customerPhone,
      deviceInfo,
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 bg-slate-950/80 backdrop-blur-sm">
      <div className="w-full max-w-md bg-slate-900 border border-slate-700 rounded-2xl p-5 space-y-4 text-white text-xs">
        <div className="flex justify-between items-center border-b border-slate-800 pb-2">
          <h3 className="font-bold text-sm">Editar Fiado #{receivable.referenceNumber}</h3>
        ...
        <form onSubmit={handleSave} className="space-y-3">
          ...
          <div>
            <label className="block text-slate-400 mb-1">Aparelho / Descrição</label>
            <input
              type="text"
              value={deviceInfo}
          ...
          <div className="flex justify-end gap-2 pt-2">
            <button type="button" onClick={onClose} className="px-3 py-2 bg-slate-800 rounded-xl">Cancelar</button>
            <button type="submit" className="px-4 py-2 bg-cyan-600 font-bold rounded-xl">Salvar</button>
          </div>
        </form>
      </div>
    </div>
...
// ==========================================
// 3. ADD PRODUCTS TO RECEIVABLE MODAL
// ==========================================
...
const AddProductsToReceivableModal: React.FC<AddProductsToReceivableModalProps> = ({
  isOpen,
  receivable,
  onClose,
  onSuccess,
  ...
  const handleAdd = (e: React.FormEvent) => {
    ...
    const updatedRec = {
      ...
      notes: `${receivable.notes || ''}\n+ Add ${quantity}x ${product.name} (R$ ${addedAmount.toFixed(2)})`.trim(),
    };
    const updated = list.map((r) => (r.id === receivable.id ? updatedRec : r));
    StorageService.saveReceivables(updated);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 bg-slate-950/80 backdrop-blur-sm">
      <div className="w-full max-w-md bg-slate-900 border border-slate-700 rounded-2xl p-5 space-y-4 text-white text-xs">
        <div className="flex justify-between items-center border-b border-slate-800 pb-2">
          <h3 className="font-bold text-sm">Adicionar Produto ao Fiado #{receivable.referenceNumber}</h3>
          <button onClick={onClose} className="p-1 text-slate-400 hover:text-white">✕</button>
        </div>
        <form onSubmit={handleAdd} className="space-y-3">
          ...
          <div className="flex justify-end gap-2 pt-2">
            <button type="button" onClick={onClose} className="px-3 py-2 bg-slate-800 rounded-xl">Cancelar</button>
            <button type="submit" disabled={!selectedProductId} className="px-4 py-2 bg-cyan-600 disabled:opacity-50 font-bold rounded-xl">Adicionar</button>
          </div>
        </form>
      </div>
    </div>
...
const ManualReceivableModal: React.FC<ManualReceivableModalProps> = ({
  isOpen,
  onClose,
  onSuccess,
}) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!customerName || numAmount <= 0) return;

    StorageService.addManualReceivable({
      customerName,
      customerPhone,
      deviceInfo: deviceInfo || 'Avulso',
      serviceDescription: 'Lançamento manual de fiado',
      amount: numAmount,
      dueDate,
    });
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 bg-slate-950/80 backdrop-blur-sm">
      <div className="w-full max-w-md bg-slate-900 border border-amber-500/40 rounded-2xl p-5 space-y-4 text-white text-xs">
        <div className="flex justify-between items-center border-b border-slate-800 pb-2">
          <h3 className="font-bold text-sm text-amber-400">Novo Fiado / Cliente Devedor</h3>
          <button onClick={onClose} className="p-1 text-slate-400 hover:text-white">✕</button>
        </div>
        <form onSubmit={handleSubmit} className="space-y-3">
          ...
          <div>
            <label className="block text-slate-400 mb-1">Valor Total Débito (R$) *</label>
            <input
              type="number"
              step="0.01"
              required
          ...
          <div className="flex justify-end gap-2 pt-2">
            <button type="button" onClick={onClose} className="px-3 py-2 bg-slate-800 rounded-xl">Cancelar</button>
            <button type="submit" className="px-4 py-2 bg-amber-600 font-bold rounded-xl text-white">Criar Fiado</button>
          </div>
        </form>
      </div>
    </div>
...
const ReceivablePayModal: React.FC<ReceivablePayModalProps> = ({
  isOpen,
  receivable,
  onClose,
  onSuccess,
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-2 sm:p-3 bg-slate-950/80 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="w-full max-w-lg bg-slate-900 border border-slate-700/80 rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[96vh]">
        <div className="flex items-center justify-between px-4 py-3 bg-slate-800/90 border-b border-slate-700">
          ...
          <button onClick={onClose} className="p-1 text-slate-400 hover:text-white hover:bg-slate-700/60 rounded-lg transition-colors">
            <X size={18} />
          </button>
        </div>
          ...
          <div className="p-2.5 rounded-xl bg-slate-800/80 border border-slate-700/80 space-y-2">
            <div className="p-2 bg-slate-900/90 border border-teal-500/30 rounded-lg">
              ...
              <p className="text-xs font-semibold text-white">
                {receivable.serviceDescription || receivable.deviceInfo || `Débito Ref: ${receivable.referenceNumber}`}
              </p>
            ...
            <div className="py-3 text-center space-y-3">
              ...
              <div className="flex flex-col sm:flex-row gap-2 pt-1">
                <button
                  ...
                  className="flex-1 py-2.5 px-3 bg-emerald-600 hover:bg-emerald-500 text-white font-bold rounded-xl flex items-center justify-center gap-1.5 transition-all cursor-pointer"
                >
                  <Send size={16} />
                ...
                <button
                  type="button"
                  onClick={onClose}
                  className="py-2.5 px-4 bg-slate-800 hover:bg-slate-700 text-slate-300 rounded-xl font-medium transition-colors cursor-pointer"
                >
                  Fechar
                </button>
              </div>
            ...
            <div className="space-y-3">
              <div className="space-y-2">
                {paymentRows.map((row, index) => (
                  <div key={row.id} className="p-2 bg-slate-900/90 border border-slate-700/80 rounded-xl space-y-1.5">
                    <div className="flex items-center justify-between text-[11px]">
                      <span className="font-bold text-emerald-400">Forma #{index + 1}</span>
                      {paymentRows.length > 1 && (
                        <button type="button" onClick={() => handleRemovePaymentRow(row.id)} className="text-slate-500 hover:text-rose-400 p-0.5 rounded cursor-pointer">
                          <Trash2 size={13} />
                        </button>
                      )}
                    </div>
                    <div className="grid grid-cols-12 gap-2 items-center">
                      ...
                      <div className="col-span-5 relative flex items-center gap-1">
                        <input
                          ...
                          className="w-full px-2 py-1.5 bg-slate-950 border border-slate-700 focus:border-emerald-400 rounded-lg text-white text-xs font-mono font-bold focus:outline-none"
                        />
                        <button
                          type="button"
                          onClick={() => handleFillRemainingInRow(row.id)}
                          className="shrink-0 px-1.5 py-1.5 bg-slate-800 hover:bg-emerald-500/20 text-emerald-300 border border-slate-700 rounded-lg text-[10px] font-semibold cursor-pointer"
                        >
                          Tudo
                        </button>
                      </div>
              ...
              >
                <Plus size={15} />
                <span>+ Outra Forma de Pagamento</span>
              </button>
              ...
              <div className="flex items-center justify-between gap-2 pt-2 border-t border-slate-700/60">
                {onDelete ? (
                  <button
                    type="button"
                    onClick={() => { onClose(); onDelete(receivable); }}
                    className="px-3 py-2 text-rose-400 hover:text-rose-300 rounded-xl text-xs font-semibold flex items-center gap-1 transition-colors cursor-pointer"
                  >
                    <Trash2 size={14} /> Excluir
                  </button>
                ) : <div />}
                <div className="flex items-center gap-2">
                  <button type="button" onClick={onClose} className="px-3.5 py-2 bg-slate-800 text-slate-300 text-xs font-semibold rounded-xl cursor-pointer">Cancelar</button>
                  <button
                    type="button"
                    onClick={handlePay}
                    disabled={loading || sumPaid <= 0}
                    className="px-5 py-2 bg-emerald-600 hover:bg-emerald-500 disabled:opacity-50 text-white text-xs font-bold rounded-xl shadow-lg flex items-center gap-1.5 transition-all cursor-pointer"
                  >
                    <CheckCircle2 size={15} /> Confirmar {formatCurrency(sumPaid)}
                  </button>
                </div>
              </div>
            </div>
