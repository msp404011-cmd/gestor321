      ...
      {activeTab === 'BACKUP' && (
        <div className={`p-6 rounded-2xl border-2 space-y-6 transition-all ${
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div className={`p-5 rounded-2xl border flex flex-col justify-between gap-4 ${
              isDark ? 'bg-[#040a17] border-cyan-900/60' : 'bg-cyan-50/50 border-cyan-200'
            }`}>
              <div>
                <h4 className={`font-bold text-sm ${isDark ? 'text-white' : 'text-slate-900'}`}>Integração Google Drive</h4>
                <p className={`text-xs mt-1 ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>
                  Para habilitar backups automáticos e login seguro na nuvem, você deve permitir o acesso do sistema ao Google.
                </p>
              </div>
              <button
                type="button"
                className="w-full py-2.5 bg-white text-slate-900 font-bold text-xs rounded-xl shadow-[0_0_12px_rgba(6,182,212,0.2)] border border-slate-200 flex items-center justify-center gap-2 cursor-pointer hover:bg-slate-50"
                onClick={() => {
                   // This acts purely as a UI stub indicating the action since the scope setup was mocked/prevented in preview.
                   alert('O acesso via Google Drive foi solicitado na camada do desenvolvedor.');
                }}
              >
                ...
                <span>Habilitar Login / Backup Google</span>
              </button>
            </div>
